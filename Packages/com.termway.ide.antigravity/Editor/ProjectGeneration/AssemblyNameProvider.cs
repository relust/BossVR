...
namespace Antigravity.Editor 
{
    ...
    {
        private readonly Dictionary<string, UnityEditor.PackageManager.PackageInfo> _packageInfoCache = new Dictionary<string, UnityEditor.PackageManager.PackageInfo>();

        private ProjectGenerationFlag _projectGenerationFlag = (ProjectGenerationFlag)EditorPrefs.GetInt("unity_project_generation_flag", 0);

        public string[] ProjectSupportedExtensions => EditorSettings.projectGenerationUserExtensions;

        public ProjectGenerationFlag ProjectGenerationFlag
        {
            get => _projectGenerationFlag;
            private set
            {
                EditorPrefs.SetInt("unity_project_generation_flag", (int)value);
                _projectGenerationFlag = value;
            }
        }
        ...
        public UnityEditor.PackageManager.PackageInfo FindForAssetPath(string assetPath)
        {
            ...
            if (_packageInfoCache.TryGetValue(parentPackageAssetPath, out var cachedPackageInfo))
            {
                return cachedPackageInfo;
            }

            var result = UnityEditor.PackageManager.PackageInfo.FindForAssetPath(parentPackageAssetPath);
            _packageInfoCache[parentPackageAssetPath] = result;
            return result;
        }
